...
function LeaveHistory({ leaves, onStatusChange, isAdmin }) {
  if (!leaves.length) {      //If the leaves array is empty ( no leave records are present)
    return <p className="text-gray-300 mt-4">No leaves applied yet.</p>;
  }

  return (
    <div className="mt-10 bg-white/20 backdrop-blur-lg rounded-2xl shadow-lg p-6 border border-white/10 hover:shadow-blue-400/40 hover:scale-102">
      {/* heading */}
                ...
                {isAdmin && ( 
                  <td className="px-4 py-2 text-center space-x-3"> 
                    {leave.status === "Pending" ? (           // showing accept and reject button only if status is pending
                      ...
                      <span className='px-3 py-1 rounded-full text-white text-sm font-semibold'>{leave.status}</span> // else if status is not pending . actual status would be shown
                    )}
